...
describe('output.helper', function()
    ...
    it('format_value', function()
        local value = helper.__private__.format_value({}, 'test', 13.3)
        assert.is_equal(13.3, value)

        local test_collector = { fields = { test = { format = '%d' } } }
        local value = helper.__private__.format_value(test_collector, 'test', 13.3)
        assert.is_equal('13', value)
    end)
