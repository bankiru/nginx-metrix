...
describe('storage.dict', function()
  ...
  it('safe_incr', function()
    ...
    stub.new(shared_dict, 'incr').on_call_with(match._, test_key, 13).returns(nil, 'unhandled error')
    newval, err = dict.safe_incr(test_key, 13)
    assert.spy(shared_dict.incr).was.called_with(shared_dict, test_key, 13)
    assert.spy(shared_dict.incr).was.called(1)
    assert.spy(shared_dict.add).was_not.called()
